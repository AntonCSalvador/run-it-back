...
export function validateChampions(dataset: GameDataset, evidence: Evidence[], manualCatalog: ManualPlayerCatalog): void {
  ...
  const manualCards = new Map(manualCatalog.cards.map(card => [card.cardId, card]));
  if (manualCards.size !== expectedCards.size) errors.push("manual catalog cardinality");
  ...
  for (const year of [2021, 2022, 2023, 2024, 2025]) {
    ...
    if (!ROLES.every(role => derived.some(card => card.year === year && card.eligibleRoles.includes(role)))) errors.push(`year ${year} roles`);
  }
  ...
  for (const card of dataset.cards) {
    ...
    for (const key of ["playerId", "teamId", "year", "displayHandle", "mapsPlayed"] as const) if (card[key] !== expected[key]) errors.push(`raw ${key} ${card.id}`);
    if (JSON.stringify(card.sourceIds) !== JSON.stringify(expected.sourceIds)) errors.push(`card sources ${card.id}`);
    const manual = manualCards.get(card.id);
    if (!manual) { errors.push(`manual card ID ${card.id}`); continue; }
    ...
    for (const trait of ["firepower", "utility", "survival", "clutch", "consistency", "leadership"] as const) if (card.traits[trait] !== manual.traits[trait]) errors.push(`manual ${trait} ${card.id}`);
    if (!entry) { errors.push(`evidence mismatch ${card.id}`); continue; }
    if (entry.mapsPlayed !== expected.mapsPlayed || entry.year !== expected.year) errors.push(`evidence mismatch ${card.id}`);
    ...
    const expectedThreshold = Math.max(2, Math.ceil(expected.mapsPlayed * 0.2));
    if (entry.threshold !== expectedThreshold) errors.push(`threshold ${card.id}`);
    const classes = Object.values(entry.agentClassMaps);
    if (classes.some(value => !Number.isInteger(value) || value < 0) || classes.reduce((sum, value) => sum + value, 0) !== expected.mapsPlayed) errors.push(`class counts ${card.id}`);
    ...
    if (entry.finalEligibleRoles.join(",") !== expected.eligibleRoles.join(",")) errors.push(`final roles ${card.id}`);
    if (expected.eligibleRoles.includes("flex") !== suggested.includes("flex")) errors.push(`flex ${card.id}`);
    if (!entry.sourceIds.length || !entry.sourceIds.every(id => sourceIds.has(id))) errors.push(`evidence sources ${card.id}`);
    for (const role of expected.eligibleRoles.filter((role): role is Exclude<Role, "flex"> => role !== "flex")) {
    ...
    if (expected.historicalIgl && (!expected.sourceIds.includes("riot-vct-2023-awards") || !entry.sourceIds.includes("riot-vct-2023-awards") || expected.traits.leadership !== 75)) errors.push(`IGL evidence ${card.id}`);
    if (!expected.historicalIgl && expected.traits.leadership !== 50) errors.push(`neutral leadership ${card.id}`);
    if (!Number.isInteger(entry.clutchCoverageMaps) || entry.clutchCoverageMaps < 0 || entry.clutchCoverageMaps > expected.mapsPlayed || !Number.isInteger(entry.clutchWins) || entry.clutchWins < 0 || entry.clutchWins > entry.clutchCoverageMaps * 5 || !entry.clutchSourceIds.length || !entry.clutchSourceIds.every(id => sourceIds.has(id))) errors.push(`clutch evidence ${card.id}`);
    if (entry.clutchCoverageMaps < expected.mapsPlayed && expected.traits.clutch !== 50) errors.push(`clutch coverage ${card.id}`);
