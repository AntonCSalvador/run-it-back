...
export function assetUrl(path: string | null): string | null {
  ...
  const local = base && path.startsWith(`${base}/assets/`) ? path.slice(base.length) : path;
...
function localAsset(path: string): boolean {
  ...
  return decoded.startsWith("/assets/") && !/[\\\u0000-\u001f?#]/.test(decoded) && !decoded.split("/").some(part => part === "." || part === "..");
}
