...
export function RosterBar({ slots, onMove }: RosterBarProps) {
  return <section aria-label="Roster" className="roster-bar">{ROLES.map(role => {
    ...
    return <div key={role}><strong>{role}</strong>{card && <><span>{card.displayHandle}</span><div aria-label={`Move ${card.displayHandle}`}>{compatibleTargets.map(target => <button type="button" key={target} onClick={() => onMove(card.id, target)}>Move {card.displayHandle} to {target}</button>)}</div></>}</div>;
  })}</section>;
}
