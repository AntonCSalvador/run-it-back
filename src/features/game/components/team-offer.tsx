...
export interface TeamOfferProps { teams: readonly [TeamAppearance, TeamAppearance, TeamAppearance]; rerolls: number; onChoose(id: string): void; onReroll(): void }
export function TeamOffer({ teams, rerolls, onChoose, onReroll }: TeamOfferProps) {
  ...
  return <section aria-labelledby="team-offer-title"><h2 id="team-offer-title">Choose a team</h2><p aria-live="polite">{rerolls} rerolls remaining</p>
    <div>{teams.map(team => <button type="button" key={team.id} onClick={() => onChoose(team.id)}><MediaMark src={team.logo} alt={`${team.name} ${team.year} logo`} label={team.shortName} />{team.name} {team.year}</button>)}</div>
